...
for (const email of [
  ...
  test(`${email}: видит «Работа», не видит «Управление»`, async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: 'Пользователи' })).toHaveCount(
      0,
    );
    await expect(page.getByRole('link', { name: 'Организации' })).toHaveCount(
      0,
    );

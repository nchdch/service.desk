...
- Docker/Docker Compose для деплоя на сервер (10.3.0.88) — отдельный вопрос, не блокирует разработку.
- Бизнес-сущности Ticket, Asset и т.д. — появятся в соответствующих подпроектах.

**Уточнение окружения (выявлено при планировании):** Docker недоступен на машине разработки — это VMware VM без nested-виртуализации (тот же блокер, что и для WSL2, см. память по telegram-ai-agent). Поэтому PostgreSQL для локальной разработки устанавливается нативно на Windows, без Docker.
...
- PostgreSQL 16 устанавливается нативно на Windows (через `winget`), создаётся выделенная база и пользователь для проекта.
- `backend/.env.example` — `DATABASE_URL`, `JWT_SECRET`, `JWT_EXPIRES_IN`, `PORT`, `SEED_USER_PASSWORD`.
- Локальный запуск: установка PostgreSQL (один раз) → `cd backend && npm install` → `npx prisma migrate dev` → `npx prisma db seed` → `npm run start:dev`.

## Критерии готовности (definition of done)

1. PostgreSQL запущен как служба Windows, миграции применяются без ошибок, сидинг создаёт организацию и 4 пользователей (по одному на роль).

...
- Docker/Docker Compose — отдельный вопрос, не блокирует разработку (см. ниже про окружение).
- Бизнес-сущности Ticket, Asset и т.д. — появятся в соответствующих подпроектах.

**Уточнение окружения (выявлено при планировании):** Docker недоступен на машине разработки (Windows) — это VMware VM без nested-виртуализации (тот же блокер, что и для WSL2, см. память по telegram-ai-agent). По решению пользователя backend-фундамент разрабатывается и запускается напрямую на сервере 10.3.0.88 (`vrtvs01`, Ubuntu 24.04, тот же сервер, что и для будущего деплоя) — там PostgreSQL и Node.js устанавливаются нативно через apt/NodeSource, без Docker. Редактирование кода — локально, синхронизация с сервером через git (репозиторий публичный на GitHub).
...
- Сервер 10.3.0.88 (`vrtvs01`, Ubuntu 24.04, пользователь `vrtadmin`, доступ по SSH-ключу) — единственное окружение для установки зависимостей, запуска и тестов backend-фундамента.
- На сервере устанавливаются Node.js 20 LTS (через NodeSource) и PostgreSQL 16 (через apt), создаётся выделенная база и пользователь PostgreSQL для проекта.
- Репозиторий клонируется на сервере (`git clone`, публичный GitHub-репозиторий); код редактируется локально и попадает на сервер через `git push` (локально) → `git pull` (на сервере).
- `backend/.env` (на сервере, не в git) — `DATABASE_URL`, `JWT_SECRET`, `JWT_EXPIRES_IN`, `PORT`, `SEED_USER_PASSWORD`; в репозитории — `backend/.env.example`.
- Запуск: установка Node.js/PostgreSQL (один раз) → `git clone`/`git pull` → `cd backend && npm install` → `npx prisma migrate dev` → `npx prisma db seed` → `npm run start:dev`.

## Критерии готовности (definition of done)

1. PostgreSQL запущен как служба на сервере 10.3.0.88, миграции применяются без ошибок, сидинг создаёт организацию и 4 пользователей (по одному на роль).
